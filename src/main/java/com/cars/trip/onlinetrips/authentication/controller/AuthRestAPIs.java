...
import com.cars.trip.onlinetrips.authentication.message.request.SignUpForm;
import com.cars.trip.onlinetrips.authentication.message.response.JwtResponse;
import com.cars.trip.onlinetrips.authentication.message.response.ResponseMessage;
		...
		if(signUpRequest.getRole() != null) {
			...
			strRoles.forEach(role -> {
				...
				case "admin":
					...
					break;
				case "em":
					Role pmRole = roleRepository.findByName(RoleName.ROLE_EM)
							.orElseThrow(() -> new RuntimeException("Fail! -> Cause: User Role not find."));
					roles.add(pmRole);
